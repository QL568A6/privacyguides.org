...
!!! recommendation
    ...
    For models which support HOTP and TOTP, there 2 slots in the OTP interface which could be used for HOTP and 32 slots to store TOTP secrets. These secrets are stored encrypted on the key and never expose them to the devices they are plugged into. Once a seed is given to the Yubico Authenticator, it will only give out the six-digit codes, but it will never give out the actual seed again. 
